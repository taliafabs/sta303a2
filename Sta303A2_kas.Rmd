...
matlines(
  x$date,
  exp(do.call(cbind, predict(res, se.fit = TRUE)) %*% Pmisc::ciMat()),
  lty = c(1, 2, 2), col = 'black'
)

newX <- x
newX$month <- 'Apr'
newX$logMonthDays <- log(30)

matlines(
  x$date,
  exp(do.call(cbind, predict(res, newX, se.fit = TRUE)) %*% Pmisc::ciMat()),
  lty = c(1, 2, 2), col = 'green', lwd = c(3, 1, 1)
)
