...
# seasonal
matlines(x$date, 
         exp(do.call(cbind, predict(model1, se.fit=TRUE)) %*% Pmisc::ciMat()),
...
matlines(x$date,
         exp(do.call(cbind, predict(model1, newX, se.fit=TRUE)) %*% Pmisc::ciMat()),
         lty = c(1, 2, 2), 
         col='red3', 
         lwd = c(1.5, 1, 1)
         )
# prediction rate mu_i (red), seasonally adjusted trend (black) with 95% prediction intervals
```
